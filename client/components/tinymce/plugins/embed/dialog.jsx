...
export class EmbedDialog extends React.Component {
	...
	componentWillMount() {
		 ...
		 *        reuse EmbedViewManager/EmbedView, but that leads to a complicated mess. When
		...
		this.debouncedUpdateEmbedPreview = debounce( function() {
			...
			wpcom.undocumented().site( this.props.siteId ).embeds(
				...
				( error, data ) => {
					...
					this.setState( {
						// The value of previewMarkup is later used with dangerouslySetInnerHtml,
						// so it must never be changed to include untrusted data.
	...
	render() {
		...
		return (
			...
			>
				...
				{/*
				iframe works, is probably redundant since the API markup always returns things in an iframe anyway?
				...
				need to use ResizeableIframe to deal w/ unknown dimentions of various embeds?
		...
		{/*
			test various embed services, both whitelisted and generic oembed
				- audio - out of scope, #1732 istead
				- instagram - renders fine in editor but not in dialog.
				 ` data.result` is just `<a data-pin-do="embedBoard" href="https://www.pinterest.com/etsy/diy-projects/"></a>`
				 maybe there's some script that runs to change those links into preview, and i needto manually call it?
				    components/embed-container ?
				- facebook - same as instagram
					`data.result` is "<fb:post href="https://www.facebook.com/WordPress/posts/10155517359162911" data-width="552"></fb:post>"

			bug: open dialog, looks good. then cancel, then open again, now it doesn't render at all.
				repo w/ pinterest & fb, maybe others from embedcotainer?
